...
import {
  BatchShipmentCountReq,
  ListBatchShipmentsReq,
  ShipmentBatchInfo
...
export const BatchView: React.VFC<Props> = () => {
  const { user } = useAuth0();
  ...
  const [batchShipments, setBatchShipments] = useState<ShipmentBatchInfo.AsObject[]>([]);
  const [nextStepButtonValue, setNextStepButtonValue] = useState('');

  useEffect(() => {
    localStorage.setItem('shipment_pagination', JSON.stringify([""]));
    setStepButtonValue('');
  ...
  useEffect(() => {
    localStorage.setItem('shipment_row', selectedRowsCount.value.toString());
    setSelectedRows(selectedRowsCount.value);
  }, [selectedRowsCount]);

  useEffect(() => {
    const rows = localStorage.getItem('shipment_row');
    if (rows) {
  ...
  useEffect(() => {
    if (companyId) {
      getListBatches()
        .then((data: any) => {
          console.log(data);
          const shipments = data[0].shipmentsList;
          setBatchShipments(shipments);
  ...
  const getListBatches = () => {
    return new Promise<any>((resolve, reject) => {
      getClient()
        .then((client) => {
          const requestBatchShipment: ListBatchShipmentsReq.AsObject = {
            companyId: companyId,
            paginationCursor: stepButtonValue,
            pageSize: selectedRows,
            batchId: params.id,
          };
          const requestBatchShipmentCount: BatchShipmentCountReq.AsObject = {
            companyId: companyId,
            batchId: params.id,
          };

          Promise.all([
            client.listBatchShipments(requestBatchShipment),
            client.batchShipmentsCount(requestBatchShipmentCount),
          ]).then(data => {
            resolve(data);
          }, error => {
            reject(error);
  ...
  const onNext = () => {
    if (loaded) {
      setLoaded(false);
      next();
      const paginationHistoryString = localStorage.getItem('shipment_pagination');
      const paginationHistory: string[] = JSON.parse(paginationHistoryString ? paginationHistoryString : `[""]`);
      localStorage.setItem('shipment_pagination', JSON.stringify([...paginationHistory, nextStepButtonValue]));
  ...
  const onPrev = () => {
    if (loaded) {
      ...
      localStorage.setItem('shipment_pagination', JSON.stringify(newHistory));
  ...
  return (
    <PlacementCard>
      ...
      <PlacementCard.Content>
        <TooltipsList>
          <ShipmentList handleChangeEvent={() => setHandleChanged(!handleChanged)} batchShipments={batchShipments}/>
        </TooltipsList>
      </PlacementCard.Content>
