...
export const AddEBaySection: React.VFC<Props> = ({ onClose }) => {
  ...
  useEffect(() => {
    if(window.location) {
      const urls = window.location.href.split(AppRoutes.Auth.Welcome.Step1.EBay + '?code=');
      setCode(urls[1]);
    }
  }, [window.location])
  ...
  return (
    <AddStoreModal storeName={store} nextPath={AppRoutes.Auth.Welcome.Step2} onClose={onClose} data={{ name: name, country: selectedCountry, code: code }}>
       {
        authenticated ?
          <ListSection>
