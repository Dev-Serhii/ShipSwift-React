...
import { API_URL, convertTime, getAuth0Token } from '../../../../../utils';
...
export const AccountPage: React.VFC<Props> = () => {
  ...
  return (
    <DisplayRound overflow>
      <styled.BodyWrapper>
        <CompanyLogoSection url={user?.picture as string} />
        <FormCaption caption={word('global.name')}>
          <InputCustom value={user?.name} />
        </FormCaption>
        <FormCaption caption={word('global.email')}>
          <InputCustom value={user?.email} />
        </FormCaption>
        ...
        <FormCaption caption={word('global.phone-number')}>
          <InputCustom value={userData.phone_number} />
        </FormCaption>
